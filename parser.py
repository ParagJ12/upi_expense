...
def _parse_navi_pdf_text(text: str, source: str) -> pd.DataFrame:
    ...
    09 Dec 2025 UPI/ZOMATO/...               ₹250.00   Dr
    """
    records = []
    lines = text.split("\n")
    ...
    for line in lines:
        ...
        if m:
            date_str, desc, amt_str, dr_cr = m.group(1).strip(), m.group(2).strip(), m.group(3), m.group(4).upper()
            ...
            if "DR" in dr_cr:
                amt = -amt
            records.append({"date": dt, "merchant_raw": desc or "Unknown", "amount": amt, "source": source})
    
    # Also try multi-line: date on one line, desc+amount on next
    if len(records) < 3:
        records = _parse_two_line_format(lines, source)
...
def _parse_gpay_pdf_text(text: str, source: str) -> pd.DataFrame:
    """
    Google Pay PDF: often has lines like
    Dec 09, 2025  Zomato  ₹250  Paid
    or 
    09/12/2025   UPI payment to Zomato  -250.00
    """
    records = []
    ...
    for line in lines:
        ...
        if m:
            ...
            if dt is None:
                continue
            amt_clean = re.sub(r'[₹Rs,\s]', '', amt_str)
            try:
                amt = float(amt_clean)
                # If no sign, check for "Paid"/"Received" in line
                if amt > 0 and "received" not in line.lower() and "credit" not in line.lower():
                    amt = -abs(amt)
            except:
                continue
